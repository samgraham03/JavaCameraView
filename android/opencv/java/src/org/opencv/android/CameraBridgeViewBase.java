...
public abstract class CameraBridgeViewBase extends SurfaceView implements SurfaceHolder.Callback {
    ...
    protected void deliverAndDrawFrame(CvCameraViewFrame frame) {
        ...
        if (bmpValid && mCacheBitmap != null) {
            ...
            if (canvas != null) {
                ...
                // Custom Rotate Canvas
                if (getResources().getConfiguration().orientation == Configuration.ORIENTATION_PORTRAIT) {
                    canvas.save();
                    canvas.rotate(90,  (canvas.getWidth()/ 2),(canvas.getHeight()/ 2));
                }
                if (getResources().getConfiguration().orientation == Configuration.ORIENTATION_LANDSCAPE && this.getDisplay().getRotation() == Surface.ROTATION_0) {
                    canvas.save();
                    canvas.rotate(270,  (canvas.getWidth()/ 2),(canvas.getHeight()/ 2));
                }
                if (getResources().getConfiguration().orientation == Configuration.ORIENTATION_LANDSCAPE && this.getDisplay().getRotation() == Surface.ROTATION_270) {
                    canvas.save();
    ...
    protected Size calculateCameraFrameSize(List<?> supportedSizes, ListItemAccessor accessor, int surfaceWidth, int surfaceHeight) {
        ...
        // Custom Rotate Surface (Resizes frame if in portrait mode)
        if (getResources().getConfiguration().orientation == Configuration.ORIENTATION_PORTRAIT) {
            int tempDim = maxAllowedHeight;
            maxAllowedHeight = maxAllowedWidth;
            maxAllowedWidth = tempDim;
        }
